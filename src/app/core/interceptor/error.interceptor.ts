...
export class ErrorInterceptor implements HttpInterceptor {
  ...
  intercept(
    ...
    return next.handle(request).pipe(
      catchError((err: { status: number; error: { error: string } }) => {
        console.log('request', request.headers.get(HTTP_HEADERS.X_IGNORE_ERROR));
        ...
        } else if (request.headers.get(HTTP_HEADERS.X_IGNORE_ERROR) !== BOOLEAN_STRING.TRUE) {
    ...
    if (!this.isTokenRefreshing) {
      ...
      return this.authService.refreshToken(refreshToken).pipe(
        switchMap((res: any) => {
          ...
          this.storeToken(accessToken, newRefreshToken);
  ...
  private storeToken(accessToken?: string, newRefreshToken?: string): void {
    this.authService.storeToken(accessToken, newRefreshToken);
  }
}

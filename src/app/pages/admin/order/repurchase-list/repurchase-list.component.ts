...
import { StatusEnum, IOrder, OrderType, PaymentMethod } from '../../../../shared/models/order.model';
...
@Component({
  selector: 'app-repurchase-list',
  templateUrl: './repurchase-list.component.html',
  styleUrls: ['./repurchase-list.component.css']
})
export class RepurchaseListComponent implements OnInit {
...
onChangeRangePrice(): void {
  ...
  create(): void {
    this.router.navigate([ROUTER_UTILS.order.root,ROUTER_UTILS.order.repurchase,ROUTER_ACTIONS.create]);
  }
  createBefore48h(){
    this.router.navigate([ROUTER_UTILS.order.root,'list',ROUTER_UTILS.order.repurchase]);
